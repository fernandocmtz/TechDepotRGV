...
import { User, CreditCard, Home, Package, PlusCircle, Trash2, Edit, Save } from 'lucide-react';
...
const Profile = () => {
      ...
      setTimeout(() => {
        ...
        if (editingPayment !== null) {
          ...
          const updatedPayments = userData.paymentMethods.map(payment => 
            payment.id === editingPayment ? { ...payment, ...newPayment } : payment
          );
        ...
        toast({
          ...
          description: editingPayment !== null ? 'Your payment method has been updated.' : 'Your payment method has been added.',
        });
      ...
      setTimeout(() => {
        ...
        if (editingAddress !== null) {
          ...
          const updatedAddresses = userData.addresses.map(address => 
            address.id === editingAddress ? { ...address, ...newAddress } : address
          );
        ...
        toast({
          ...
          description: editingAddress !== null ? 'Your address has been updated.' : 'Your address has been added.',
        });
  ...
  const deletePaymentMethod = (id: number) => {
    setUserData({
      ...userData,
      paymentMethods: userData.paymentMethods.filter(payment => payment.id !== id),
    });
            ...
            <TabsContent value="payment-methods" className="space-y-4 mt-4">
              {!addingPayment ? (
                ...
                <Card>
                  <CardHeader>
                    <CardTitle>
                      {editingPayment !== null ? 'Edit Payment Method' : 'Add New Payment Method'}
                    </CardTitle>
                  </CardHeader>
                  <CardContent>
                    <Form {...paymentForm}>
